    ...
    <div className='h-full content-center grid justify-items-center'>
      ...
      <form onSubmit={submitLoginForm} >
        <div className='mb-4'>
          ...
          <input onChange={onChange} value={loginData.email} className='h-18 shadow-custom w-64 bg-primary border-2 rounded-md border-solid border-stone-500 p-2' type="email" id="email" name="email" placeholder='e-mail' />
        </div>
        <div className='mb-8'>
          {errors.password && <p className="text-primaryDark text-sm text-red-700">{errors.password}</p>}
          <input onChange={onChange} value={loginData.password} className='h-18 shadow-custom w-64 bg-primary border-2 rounded-md border-solid border-stone-500 p-2' type="password" id="password" name="password" placeholder='password' />
        </div>
        {/* Login Button ======= */}
